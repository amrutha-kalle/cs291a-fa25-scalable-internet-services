...
class MessageTest < ActiveSupport::TestCase
  ...
  test "message should be tied to convo" do
    message = Message.new(sender: @user, sender_role: 'initiator', content: "test message", is_read: false)
    assert_not message.save, "invalid message should not be saved"
  end

  test "message must have content, sender, (valid) sender_role" do
    message1 = Message.new(conversation: @conversation, sender: @user, content: "test", is_read: false)
    assert_not message1.save, "msg should not save without sender role"
    message2 = Message.new(conversation: @conversation, sender: @user, sender_role: "initiator", is_read: false)
    assert_not message2.save, "msg should not save without content"
    message3 = Message.new(conversation: @conversation, sender: @user, sender_role: "middleman", content: "test", is_read: false)
    assert_not message3.save, "msg should not save without valid sender role"
    message4 = Message.new(conversation: @conversation, sender: @user, sender_role: "expert", content: "test", is_read: false)
    assert message4.save, "msg should have been saved with expert role"
  end

  test "all attributes" do
    message = Message.create!(conversation: @conversation, sender: @user, sender_role: "initiator", content: "test", is_read: false)
